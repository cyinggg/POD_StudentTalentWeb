...
def admin_slot_control():
    ...
    # --- Calculate approved shifts from applications ---
    if not app_df.empty:
        app_df["date"] = pd.to_datetime(app_df["date"], errors="coerce")
        app_df["approvedshift"] = app_df.get("admindecision", "").apply(
    ...
    # --- Build calendar structure (Monday start) ---
    ...
    for week in cal.monthdatescalendar(year, month):
        ...
        for d in week:
            shifts = (
                ...
                .copy()
                if d.month == month else pd.DataFrame()
            )

            # --- Add eligibility for booking (OJT / NightShift) ---
            shifts_list = []
            for _, slot in shifts.iterrows():
                ...
                # --- OJT eligibility ---
                if slot["onjobtrain"] == 1:
                    # Only OJT users can book this slot
                    pass  # logic enforced at booking stage

                # --- NightShift eligibility ---
                if slot["nightshift"] == 1:
                    pass  # logic enforced at booking stage

                shifts_list.append(slot.to_dict())

            week_data.append({"date": d, "shifts": shifts_list})
        weeks.append(week_data)

    # --- Rename keys for template display (keep Morning/Afternoon/Night headers) ---
    slot_df_month_display = slot_df_month.rename(columns={
        "shiftperiod": "shiftPeriod",
        "shiftlevel": "shiftLevel",
        "approvedshift": "approvedShift",
        "isopen": "isOpen",
    ...
    return render_template(
        ...
        year=year,
        weeks=weeks,
        slots=slot_df_month_display.to_dict("records"),  # template uses title-case keys
